...
class Looper:
    ...
    async def _loop_exec(self, command: BaseCommand):
    ...
    async def loop_if(self, command: BaseCommand, result: Result):
        if command.loop_if and (match := re.search(command.loop_if, result.stdout, re.MULTILINE)):
            ...
            loop_limit = CmdVars.variable_to_int('loop_count', command.loop_count)
            if self.run_count < loop_limit:
                self.run_count += 1
                await asyncio.sleep(self.cmdconfig.loop_sleep)
                await self._loop_exec(command)
            else:
    ...
    async def loop_if_not(self, command: BaseCommand, result: Result):
